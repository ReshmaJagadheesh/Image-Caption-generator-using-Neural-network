...
test_img_list=[i[0] for i in test_list]
...
test_dict={i[0]:i[1] for i in test_list}
...
for (j,i) in test_dict.items():
    test_token.append([j,nltk.word_tokenize(i.lower())])
...
with io.open('flickr30k_text/train_captions.txt','w',encoding='utf-8') as f:
...
with io.open('flickr30k_text/val_captions.txt','w',encoding='utf-8') as f:
...
##Resize image
image_dir = 'flickr30k_image/'
output_dir = 'flickr30k_resized_image/'
image_size = [256,256]
    ...
    for i, (images,captions,lengths) in enumerate(train_dataloader):
        ...
        if (i+1) % 1000 == 0:
            torch.save(decoder.state_dict(), os.path.join(
                    'models/flickr30k/', 'decoder-{}-{}.ckpt'.format(epoch+1, i+1)))
            torch.save(encoder.state_dict(), os.path.join(
                    'models/flickr30k/', 'encoder-{}-{}.ckpt'.format(epoch+1, i+1)))'''
...
torch.save(encoder,os.path.join('models/flickr30k/','encoder.model'))
torch.save(decoder,os.path.join('models/flickr30k/','decoder.model'))


# In[101]:


with open('flickr30k_text/train_loss_30k.txt','w') as f:
    for i in train_loss:
        f.write(str(i.item()))
        f.write(' ')
...
image = load_image('flickr30k_image/'+ str(file), transform_test)
...
print (sentence)
image = Image.open('flickr30k_image/'+ str(file))
...
with open('flickr30k_text/test_loss_30k.txt','w') as f:
    for i in test_loss:
        f.write(str(i.item()))
        f.write(' ')
...
with open('flickr30k_text/ref_30k.txt','w') as f:
...
with open('flickr30k_text/hyp_30k.txt','w') as f:
    for i in generated_caption:
        f.write(i[1])
        f.write('\n')
...
scores_meteor_file = "flickr30k_text/test_meteor.txt"


# In[92]:


ref_file='flickr30k_text/ref_30k.txt'
hyp_file='flickr30k_text/hyp_30k.txt'
